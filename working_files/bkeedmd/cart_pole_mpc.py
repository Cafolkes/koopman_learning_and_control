...
n_traj_train = 25                                                 # Number of trajectories to execute, data collection
...
xdmd, y_dmd = model_dmd.process(xs_train, us_train, np.tile(t_train,(n_traj_train,1)), 
                                downsample_rate=sub_sample_rate)
model_dmd.fit(xdmd, y_dmd, cv=tune_mdl_dmd, override_kinematics=False)
...
basis = lambda x: poly_basis.transform(x)
...
model_edmd = Edmd(n, m, basis, n_lift_edmd, n_traj_train, optimizer_edmd, cv=cv_edmd, 
...
X_edmd, y_edmd = model_edmd.process(xs_train, us_train, np.tile(t_train,(n_traj_train,1)), 
                                    downsample_rate=sub_sample_rate)
model_edmd.fit(X_edmd, y_edmd, cv=tune_mdl_edmd, override_kinematics=False)
...
alpha_bedmd = 1.5e-3  # Regularization strength (LASSO) bEDMD
...
standardizer_x_kdnn = None#fit_standardizer(xs_train, preprocessing.StandardScaler())
standardizer_u_kdnn = None#fit_standardizer(us_train, preprocessing.StandardScaler())
