...
class BilinearMPCTrajController(NMPCTrajController):
    ...
    def __init__(self, dynamics, N, dt, umin, umax, xmin, xmax, C_x, C_obj, Q, R, QN, xr, solver_settings,
        ...
        super(BilinearMPCTrajController, self).__init__(dynamics, N, dt, umin, umax, xmin, xmax, C_x, C_obj, Q, R, QN,
                                                        xr, solver_settings, const_offset = None,
    ...
    def update_linearization_(self):
        ...
        A_lst_flat = (self.B_flat @ self.u_init + self.A_flat).flatten(order='F')
        #B_lst_flat = (self.z_init[:-1, :] @ self.B_arr).flatten()
        B_lst_flat = (self.B_arr @ self.z_init[:, :-1]).flatten(order='F')
    ...
    def eval(self, x, t):
        u = super(BilinearMPCTrajController, self).eval(x, t)
